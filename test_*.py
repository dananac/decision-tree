...
def test_build_list():
    file_name = "data1.csv"
    testData = np.genfromtxt(file_name, dtype=str, delimiter=',')
    assert ds.build_list(testData)

# tests building dict
def test_build_dict():
    file_name = "data1.csv"
    testData = np.genfromtxt(file_name, dtype=str, delimiter=',')
    assert ds.build_dict(testData)
...
def test_train_data():
    ...
    assert dt.DT_train_binary(X,Y,max_depth)
...
def test_tree():
    ...
    assert dt.DT_test_binary(X,Y,DT)
...
def test_make_prediction():
    ...
    X,Y = ds.build_nparray(testData)
    DT = dt.DT_train_binary(X,Y,max_depth)
    assert dt.DT_make_prediction(Y,DT)
...
def test_build_tree():
    ...
    X,Y = ds.build_nparray(testData)
    DT = dt.DT_train_binary(X,Y,max_depth)
    assert dt.build_tree(X,Y,max_depth,DT)
